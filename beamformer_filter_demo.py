...
for k in range(len(beacon_pos[0])):
    ...
    for antenna in antenna_list:
        ...
        s_m = sim.measure_s_m_multipath(t=t, antenna_positions=ant_pos_m_i,
                                        ...
                                        phi_B=phi_B, sigma=0.001, multipath_sources=multipath_sources)
        ...
        for i in range(len(s_m)):
            ...
            plt.subplot(r, r, i + 1)
            plt.plot(np.abs(s_m[i, :]), label="s_m")
            plt.plot(np.abs(s_m_filtered[i, :]), label="s_m filtered")
            plt.legend()
