...
class SpatialFilter:
    ...
    def remove_close_peaks(self, theta_peak, phi_peak, sorted_maxima, eps=0.1):
        """
    ...
    def remove_components_2D(self, x, r, results, phis, thetas, output_signals, peak_threshold):
        """
    ...
    def remove_target_with_sidelobes(self, peak_thetas, peak_phis, sorted_maxima, beamformer, antenna, target_theta,
                                     target_phi,
                                     eps, peak_threshold):
        """
        ...
        u = np.array(
            [np.sin(target_theta) * np.cos(target_phi), np.sin(target_theta) * np.sin(target_phi),
             np.cos(target_theta)])
        k = -2 * np.pi * self.params.f / self.params.c * u
        ...
        # ax.set_xlim(self.params.room_x)
        # ax.set_ylim(self.params.room_y)
        # ax.set_zlim(self.params.room_z)
        #
        # ax.set_xlabel("x(m)")
        # ax.set_ylabel("y(m)")
        # ax.set_zlabel("z(m)")
    ...
    def keep_target(self, peak_thetas, peak_phis, sorted_maxima, target_theta, target_phi, cone_angle):
        """
    ...
    def remove_target(self, peak_thetas, peak_phis, sorted_maxima, target_theta, target_phi, cone_angle):
        """
    ...
    def two_step_filter(self, x, r, peak_threshold, num_of_removed_signals=None,
                        target_theta=None, target_phi=None, cone_angle=None,
    ...
    def multipath_search_filter(self, x, r, beamformer, antenna, peak_threshold, target_theta, target_phi,
                                cone_angle):
        """
    ...
    def remove_max_2D(self, x, r, results, phis, thetas, output_signals, num_of_removed_peaks,
                      peak_threshold,
                      beamformer=None, antenna=None, eps=0.1, x2=None):
        """
        ...
        :param d_phi: phi range of target
        :param peak_threshold: the threshold to count maxima as peak
        :param x2: second signal which will be filtered according to first x
        :return: fitered_x, is a peak removed
        """
        filtered_x = x.copy()
        is_filtered = False
        ...
        for k in range(0, min(num_of_removed_peaks, len(peak_thetas))):
            is_filtered = True
            theta_to_remove = peak_thetas[k]
            phi_to_remove = peak_phis[k]
            if VERBOSE:
            ...
            filtered_x -= signal_to_remove_at_antenna
            if x2 is not None:
                x2 -= signal_to_remove_at_antenna
        if VERBOSE:
            print("\n\n")

        if not is_filtered:
            return filtered_x, x2, False
        return filtered_x, x2, True

    def remove_components_1D_theta(self, x, r, results, phi, thetas, output_signals):
        filtered_x = x.copy()
        N_array = len(r[0])
    ...
    def multipath_filter(self, x, r, beamformer, antenna, peak_threshold, target_theta, target_phi, cone_angle,
                         ):
        filtered_x = x.copy()
        N_array = len(r[0])
        results, output_signals, thetas, phis = beamformer.compute_beampattern(x=x,
